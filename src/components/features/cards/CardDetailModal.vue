<template>
  ...
  >
    <div v-if="card" class="card-detail-modal">
      <div class="d-flex flex-column flex-md-row align-start ga-8">
        ...
        <div class="card-info-section flex-grow-1 d-flex flex-column">
          <h3 class="text-h4 font-weight-bold mb-6 text-primary">
            {{ card.name }}
          </h3>

          <div class="mb-8">
            <h4 class="text-h6 font-weight-medium mb-4 text-grey-darken-1">
              Descrição
            </h4>
            <p class="text-body-1 text-grey-darken-2 line-height-1-6">
              {{ card.description }}
            </p>
          </div>
    ...
    <div
      v-else-if="loading"
      class="d-flex flex-column align-center justify-center py-15 text-grey"
    >
    ...
    >
      ...
      <p class="text-body-1 text-grey">
        A carta solicitada não foi encontrada ou não existe.
      </p>
    </div>
  </BaseModal>
</template>
