...
// Schema para confirmação de ações
...
// Schema para seleção de itens
export const selectionSchema = z.object({
  selectedItems: z.array(z.string().uuid())
...
export const paginationSchema = z.object({
  ...
  itemsPerPage: z.number().int().min(1, 'Itens por página deve ser maior que 0').max(100, 'Máximo 100 itens por página'),
  totalItems: z.number().int().min(0, 'Total de itens não pode ser negativo').optional()
});
...
// Schema para formulário de modal
export const modalFormSchema = z.object({
  title: z.string().min(1, 'Título é obrigatório').max(100, 'Título muito longo'),
  description: z.string().max(500, 'Descrição muito longa').optional(),
  fields: z.array(z.object({
    name: z.string(),
    type: z.enum(['text', 'email', 'number', 'select', 'multiselect', 'date']),
...
// Schema para configurações de modal
...
// Schema para estados de loading
...
// Schema para validação de formulários dinâmicos
